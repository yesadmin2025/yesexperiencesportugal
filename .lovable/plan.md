...
## Fase 1 — Português completo (prioridade máxima)

Hoje só 13 páginas têm PT (`/`, `/about`, `/experiences`, `/contact`, legais…). As páginas de tours, Studio, day-tours de destino e Local Stories só existem em inglês.

1. Escrever copy PT-PT editorial (não tradução literal) para:
   - as 11 páginas de Signature (`/tours/$tourId`) — título, intro, highlights, itinerário, FAQ;
   - Experience Studio V3 (perguntas, microcopy, revelação final, guest details, checkout);
   - Travel Designer, Corporate, Multi-day, Moments, Trade, Search, Itinerary;
   - todos os CTAs e mensagens de erro/estado ("Reserve this day" → "Reservar este dia").
2. Mover as strings para `src/content/i18n/pt|en/*.json` por área (tours, studio, checkout) em vez de texto inline, para escalar.
3. Rotas PT em falta criadas com o mesmo padrão de `pt.about.tsx` (`/pt/tours/$tourId`, `/pt/studio`, …).
4. Registar cada rota nova em `src/i18n/pt-ready.ts` (READY + PAIRED) → hreflang recíproco e sitemap bilingue automáticos.
5. **Entrego tudo para tua revisão editorial antes de considerar fechado** — factos (paragens, preços, inclusões) nunca são inventados, só reescritos em PT.
...
## Fase 4 — Acessibilidade e mobile
