...
const geoKeys = [...geoFile.matchAll(/^\s*"?([a-zA-Z0-9\u00C0-\u017F&\-\s']+?)"?:\s*\{ lat/gm)].map(m=>m[1].toLowerCase());
// crude - just check SoT labels vs geo aliases
const norm = s => s.toLowerCase().replace(/[^a-z0-9\u00c0-\u017f\s]/gi,'').trim();
const geoSet = new Set(geoKeys.map(norm));

// SoT tour blocks
const blocks = [...sotFile.matchAll(/"([a-z\-]+)":\s*\{[\s\S]*?itinerary:\s*\[([\s\S]*?)\]\s*,\s*\n\s*durationMinutes/g)];
