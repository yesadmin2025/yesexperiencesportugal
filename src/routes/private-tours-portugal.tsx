...
export const Route = createFileRoute("/private-tours-portugal")({
  head: () => ({
    ...
    scripts: [
      jsonLdScript(jsonLd),
      jsonLdScript(breadcrumbLd([{ name: "Home", path: "/" }, { name: "Private Tours Portugal", path: PAGE_PATH }])),
    ],
  }),
  component: Page,
});

const ITEMS: { eyebrow: string; title: string; body: string; cta: { to: string; tourId?: string; label: string } }[] = [
  {
    eyebrow: "Why private",
  ...
  },
  {
    ...
    cta: { to: "/tours/$tourId", tourId: "sintra-cascais", label: "See the Sintra & Cascais Signature" },
  },
  {
        ...
        <header className="pt-32 md:pt-40 pb-12 md:pb-16 bg-[color:var(--sand)]">
          <div className="container-x max-w-3xl text-center">
            ...
            <SectionTitle as="h1" size="anchor" spacing="loose">
              ...
              <SectionTitle.Em>one family at a time.</SectionTitle.Em>
            </SectionTitle>
            ...
            <div className="mt-8 flex flex-col sm:flex-row items-center justify-center gap-4">
              <CtaButton to="/experiences" variant="primary">Explore Signature Tours</CtaButton>
          ...
          <div className="container-x max-w-2xl">
            <div className="prose-yes">
              <p className="text-[16px] md:text-[17px] text-[color:var(--charcoal)] leading-[1.85]">
                Every experience we run is <strong className="font-medium">private by default</strong>{" "}
                — never joined with strangers, never bus-scheduled. We're a licensed Portuguese
                operator (RNAAT) based in Sesimbra, so the guides, the cars, and the estates are
                ours, not a broker's.
              </p>
