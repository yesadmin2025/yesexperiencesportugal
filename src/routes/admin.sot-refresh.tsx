...
function SotRefreshPage() {
  ...
  async function runAll() {
    ...
    const snippetsById = new Map<string, string>();
    ...
    for (const id of ids) {
      ...
      for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
        ...
        if (attempt < MAX_ATTEMPTS) {
          // brief backoff before retry (0.8s, then 1.6s)
          await new Promise((r) => setTimeout(r, 800 * attempt));
        }
      }
      if (snip) snippetsById.set(id, snip);
    ...
    setCombined(header + Array.from(snippetsById.values()).join("\n"));
