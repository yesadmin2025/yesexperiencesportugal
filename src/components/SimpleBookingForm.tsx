      ...
      <div className="mt-6">
        <Field label="Date" icon={<Calendar size={14} />}>
          <input
            ...
            className={`signature-date-input block w-full min-w-0 max-w-full min-h-[48px] appearance-none border bg-[color:var(--ivory)] px-3 py-2.5 text-[16px] sm:text-sm focus:outline-none ${
              blockMessage
...
function Field({
  ...
  return (
    <div className="block" role="group" aria-labelledby={labelId}>
      <div
        id={labelId}
        className="flex items-center gap-1.5 font-medium text-[12px] uppercase tracking-[0.22em] text-[color:var(--charcoal)] mb-1.5"
      >
        {icon}
        {label}
      </div>
