...
export function YourDayTimeline({ moments, activeCount, activeIndex = null, className }: Props) {
  ...
  return (
    <ol
      ...
      className={`relative mx-auto w-full max-w-[520px] pl-10 sm:pl-8 ${className ?? ""}`}
    >
      {/* Hairline spine — the only ornament. */}
      <span
        aria-hidden
        className="absolute left-[15px] sm:left-[13px] top-2 bottom-2 w-px"
      ...
      {moments.map((m, i) => {
        ...
        return (
          <li
            ...
            className="relative pb-8 sm:pb-6 last:pb-0 motion-reduce:!opacity-100 motion-reduce:!translate-y-0"
            style={{
              opacity: visible ? 1 : 0,
          ...
          >
            ...
            <h3
              className="text-[17px] sm:text-[18px] leading-[1.25] font-semibold [text-wrap:pretty]"
            ...
            {m.story ? (
              <p
                className="mt-2.5 text-[13.5px] sm:text-[13px] leading-[1.65] sm:leading-relaxed [text-wrap:pretty]"
                style={{
