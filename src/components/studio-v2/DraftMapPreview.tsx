...
export function DraftMapPreview({
  ...
  const routeD = useMemo(() => {
    ...
    return projected.map((p, i) => (i === 0 ? `M ${p.x} ${p.y}` : `L ${p.x} ${p.y}`)).join(" ");
  }, [projected]);
  ...
  return (
    ...
    >
      {/* Header strip */}
      <div
        className="relative z-20 flex items-center justify-between gap-3 border-b px-4 md:px-5 py-2.5 md:py-3"
        style={{
          borderColor: "color-mix(in oklab, var(--gold) 15%, transparent)",
          background: "color-mix(in oklab, var(--charcoal) 90%, transparent)",
        }}
      >
        <span
      ...
      {/* Chips row */}
      <div
        className="relative z-10 flex flex-wrap items-center gap-1.5 md:gap-2 border-b px-4 md:px-5 py-3"
        style={{
          borderColor: "color-mix(in oklab, var(--gold) 12%, transparent)",
          background: "color-mix(in oklab, var(--charcoal) 70%, transparent)",
        }}
      >
      ...
      {/* Investment + CTA band */}
      <div
        className="relative z-10 border-t px-4 md:px-5 py-3.5"
        style={{
          borderColor: "color-mix(in oklab, var(--gold) 15%, transparent)",
        ...
        <div className="flex flex-wrap items-end justify-between gap-x-3 gap-y-3">
          <div className="min-w-0">
            ...
            <div className="mt-1 flex items-baseline gap-1.5">
              <span
                className="font-display text-[1.5rem] md:text-[1.85rem] leading-none font-semibold tabular-nums"
