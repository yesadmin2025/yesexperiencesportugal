    ...
    <Body style={{ backgroundColor: "#ffffff", margin: 0 }}>
      <Container style={container}>
        ...
        <Text style={{ color: "#333", fontSize: 14 }}>
          A monitorização automática detetou que todos os domínios legacy estão agora a responder
          com <strong>HTTP 410 Gone</strong> sem cabeçalho Location — a configuração DNS propagou e
          o Google vai começar a desindexar os URLs antigos.
        </Text>
          ...
          {hosts.map((h) => (
            <div key={h.host} style={{ marginTop: 12 }}>
              ...
              <Text style={value}>
                ...
                {h.aRecords && h.aRecords.length > 0 ? ` · A → ${h.aRecords.join(", ")}` : ""}
              </Text>
            </div>
          ))}
        </Section>
        ...
        <Text style={{ color: "#666", fontSize: 12, marginTop: 16 }}>
          Próximos passos: acompanhar a desindexação em Search Console (Removals já submetidos) e
          confirmar que a pesquisa Google por "Yes Experiences" deixa de mostrar o Business Profile
          antigo nas próximas 2–6 semanas.
        </Text>
      </Container>
    </Body>
  </Html>
