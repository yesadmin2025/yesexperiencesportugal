...
export const INTENT_OPTIONS: { id: IntentAtmosphere; label: string; sub: string }[] = [
  {
    ...
    sub: "Slow roads, long views, breathing room",
  },
  {
    id: "elegant_cultural",
    label: "Elegant & cultural",
    sub: "Heritage, architecture, quiet sophistication",
  },
  ...
  {
    id: "coastal_cinematic",
    label: "Coastal & cinematic",
    sub: "Cliffs, light, the Atlantic edge",
  },
];
...
> = {
  relaxed_scenic: {
    ...
    tintB: "var(--gold-soft)",
    mix: 55,
    whisper: "Wide horizons, slow light.",
  },
  elegant_cultural: {
    tintA: "var(--ivory)",
    tintB: "var(--sand)",
  ...
  food_local: {
    ...
    tintB: "var(--sand)",
    mix: 60,
    whisper: "Cellars, copper light, long lunches.",
  },
  social_celebratory: {
    tintA: "var(--gold)",
    tintB: "var(--gold-soft)",
    mix: 35,
    whisper: "A day that lifts the room.",
  },
  romantic_intimate: {
    tintA: "var(--gold-soft)",
    tintB: "var(--ivory)",
  ...
  coastal_cinematic: {
    ...
    tintB: "var(--ivory)",
    mix: 35,
    whisper: "Cliffs, salt, Atlantic gold.",
  },
};

/**
 * Reveal framing — a single Newsreader-italic phrase shown above the itinerary.
 ...
 */
export function revealFraming(intent: IntentAtmosphere | undefined, region: string): string {
  const place =
    region === "arrabida"
      ? "the Arrábida"
      : region === "lisbon-coast"
  ...
  switch (intent) {
    case "relaxed_scenic":
      return `A slow arc through ${place} — light, distance, breath.`;
    case "elegant_cultural":
      return `${place.charAt(0).toUpperCase() + place.slice(1)}, read through stone and quiet rooms.`;
    case "food_local":
      return `${place.charAt(0).toUpperCase() + place.slice(1)}, tasted unhurriedly.`;
    case "social_celebratory":
      return `A day in ${place} that lifts the occasion.`;
    case "romantic_intimate":
      return `${place.charAt(0).toUpperCase() + place.slice(1)} — designed for two.`;
    case "coastal_cinematic":
      return `Cliffs, salt and slow gold along ${place}.`;
    default:
      return `A considered day across ${place}.`;
  }
}
...
export function storyAfterIntent(intent: IntentAtmosphere): string {
  switch (intent) {
    case "relaxed_scenic":
      return "A slow, scenic thread starts to take shape.";
    case "elegant_cultural":
      return "An elegant cultural arc begins to form.";
    case "food_local":
      return "A food-led day is being shaped.";
    case "social_celebratory":
      return "A celebratory rhythm is emerging.";
    case "romantic_intimate":
      return "An intimate route is being drawn.";
    case "coastal_cinematic":
      return "A cinematic coastal route is forming.";
  }
}
...
export function storyAfterPace(pace: PaceV2): string {
  switch (pace) {
    case "light":
      return "Three considered stops. Room to breathe.";
    case "balanced":
      return "Four stops, a natural rhythm.";
    case "rich":
      return "Five stops — full, yet still elegant.";
    case "full":
      return "A dense day, edited with care.";
  }
}
...
export function storyFinalLines(
  p: {
    name?: string;
