 ...
 * the current published rows first so callers can offer a single undo.
 * Throws (never partial) — Supabase upsert with an ON CONFLICT key applies
 * per-row atomically; on error we surface it and skip the undo toast.
 */
...
/** Delete published overrides for specific slots of a module. */
 ...
 * Restores prior overrides where they existed, deletes the rest.
 */
