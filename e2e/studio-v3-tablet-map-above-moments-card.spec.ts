// Tablet-only — mirrors the mobile invariant: while the route is
// unfolding (map anticipation state, pre-reveal), the moments/story
// card must sit BELOW the map so the user can watch the route
// compose without the card occluding it.
//
// Runs only under the `tablet-chromium` Playwright project. Predictive
// skips: at tablet width the layout may legitimately place the map
// and card side-by-side; the vertical-stack invariant only applies
// when the two boxes horizontally overlap.
...
test.describe("Studio V3 tablet — moments card sits below the composing map", () => {
  ...
  test("moments card is stacked underneath the map during composition", async ({ page }) => {
    ...
    // At tablet width, a side-by-side layout is legitimate. Only enforce
    ...
    // layout choices.
    await page.screenshot({ path: "/tmp/browser/tablet-stack/tablet-map-composing.png" });

    if (!horizontallyOverlaps) {
      test.skip(true, "tablet layout places map and card side-by-side; stack invariant N/A");
      return;
    }
