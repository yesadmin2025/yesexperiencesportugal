# YES Experiences Portugal — Google Business Profile info sheet

Use this to claim, verify, and complete the Google Business Profile listing.

## Basic information

| Field | Value |
|-------|-------|
| Business name | YES Experiences Portugal |
| Business type | Service-area business (recommended) — or storefront if you want the Sesimbra base visible |
| Primary category | Tour operator |
| Additional categories | Travel agency, Tour guide, Wine tour operator, Private driver, Sightseeing tour agency |
| Address | Sesimbra, Portugal (use only if you want a public storefront address) |
| Service areas | Lisbon, Cascais, Sintra, Sesimbra, Setúbal, Azeitão, Évora, Comporta, Tróia |
| Phone | +351 911 889 992 |
| Website | https://yesexperiencesportugal.com |
| Email | info@yesexperiencesportugal.com |
| Hours | Monday–Sunday, 08:00–20:00 |
| License | RNAAT nº 31/2023 — Registered Portuguese tourism operator |

## Business description (750 characters max)

YES Experiences Portugal designs private, emotionally memorable day trips and multi-day journeys across Portugal. Based in Sesimbra, we operate as a licensed Portuguese tour operator (RNAAT nº 31/2023) and offer flexible door-to-door pickup from Lisbon, Cascais, Sintra, Sesimbra, Setúbal, Azeitão, Évora, Comporta and Tróia. Every experience is built around real local knowledge, private guiding and vehicles, and seamless booking with instant confirmation. Specialties include wine tours in Arrábida and Alentejo, coastal escapes, cultural deep-dives, and fully custom itineraries for couples, families and premium private groups.

## Services (add as many as apply)

- Private day trips from Lisbon
- Wine tours in Arrábida, Azeitão, Alentejo, and Douro
- Custom Portugal itineraries
- Multi-day private tours
- Lisbon, Sintra, Cascais, and Évora private tours
- Tailored experiences for special occasions and celebrations
- Corporate and group travel design
- Airport transfers and private driver service

## Opening hours (copy-paste)

```
Monday: 08:00–20:00
Tuesday: 08:00–20:00
Wednesday: 08:00–20:00
Thursday: 08:00–20:00
Friday: 08:00–20:00
Saturday: 08:00–20:00
Sunday: 08:00–20:00
```

## Important notes

- Create only ONE profile. Do not create separate listings for each city or region — Google treats duplicates as spam.
- If Google already shows “YES Experiences Portugal,” claim it instead of creating a new one.
- For fastest verification, choose video verification or phone verification when offered.
- After verification, upload 3–5 real photos: a team/owner photo, a vineyard or coastal stop, a vehicle interior, and a happy-guest moment.
- Add the license number (RNAAT nº 31/2023) to the business description or a post.

## First post ideas (after verification)

1. “Private day trips from Lisbon — pickup included. Explore Arrábida, Sintra, or Évora with a licensed local operator.”
2. “New for 2026: fully tailored Portugal itineraries, designed around your dates and pace.”
3. “Licensed tour operator RNAAT nº 31/2023. Based in Sesimbra, designing journeys across Portugal.”
